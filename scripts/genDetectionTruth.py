...
thresh = 0.24
flickerMargin = 2
path2images = '~/Documents/AGZ/MAV_Images/'
path2weights = '~/Documents/darknet_weights/'
path2darknet = '~/Documents/DREML/darknet-nnpack-dreml/'
...
path2images = os.path.abspath(path2images)
threshStr = '0_' + str(thresh).split('.')[-1]
path2weight = path2weights + '/' + net + '.weights'
leafDir = net + '-' + imageList.split('.txt')[0].split('AGZ_')[-1]
outputDir = os.path.abspath('../object-detection/raw/' + str(size) + '/resized/' + threshStr + '/' + leafDir + '/')
